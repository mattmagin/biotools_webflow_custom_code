...
let tableSetup = (filters, container, firstButtonName) => {
  ...
  let filterOutput = document.querySelector(container);
    filters.forEach((filter, i) => {
      ...
      let button = `<button class="button filter-button ${i==0 ? "active" : ""}" data-filter="${filter}" onclick="filterClick(this)">${i==0 ? firstButtonName : filter}</button>`;
      filterOutput.innerHTML += button;
    });
  
  hideRows((filterOutput.parentElement).querySelectorAll(".table-row .row-inner"), santiseCategoryName(filters[0]));
}